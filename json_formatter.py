...
class Formatter(MindmapExporter):
                ...
                if 'T' in datetime_str:
                    if '+' in datetime_str:
                        ...
                        if len(dt_part_list) >= 4:
                            dt_part = f"{dt_part_list[0]}-{dt_part_list[1]}-{dt_part_list[2]}"
                        else:
    ...
    def _extract_worklog_section(self, section_node: xml.Element) -> Optional[Dict[str, Any]]:
                ...
                for child in task_node:
                    if child.tag == 'node':
                        if self._is_datetime_node(child):
                            ...
                            child_has_times = any(
                                grandchild.tag == 'node' and self._is_datetime_node(grandchild)
